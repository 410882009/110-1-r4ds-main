...
```{r check-funs}
check_class <- function(){
  class_check <- vector("integer",
length(dataSet3))
  for(.x in seq_along(dataSet3)){
    class_check[[.x]] <- class(dataSet3[[.x]])
  }
  names(class_Check) <- names(dataSet3)
  class_check#the lasrt executed line
}

check_na <- function(){
  #NA check
  na_check <- vector("integer",
length(dataSet3))
  for(.x in seq_along(dataSet3)){
    na_check[[.x]] <- sum(is.na(dataSet3[[.x]]
))
  }
  names(na.check) <- names(dataSet3)
  na_check #the last executed line
}
...
```{r}
myfun <- function() xx+5 #shortcut definition
myfun <- function(){
  return(xx+5)
}
...
#there is a zz totally for different purpose
xx=c("a", "b")
myfun()
```
...
{
  for(.x in seq_along(list_string)){
    string::str_remove_all(list_string[[.x]], ' ') -> 
      list_string[[.x]]
  }
  list_string
}
